// Generated by CoffeeScript 1.6.3
(function() {
  var apiFiles, apiProvider, app, commander, express, logger, path, port, _;

  _ = require('lodash');

  path = require('path');

  commander = require('commander');

  express = require('express');

  apiProvider = require('./api_provider');

  logger = require('./logger');

  commander.version('api_faker 0.0.1').usage('[options] <file ...>').option('-p, --port <port>', 'use the specified port', '8080').option('-v, --verbose', 'verbose output');

  commander.on('--help', function() {
    console.log('  Examples:');
    console.log('');
    console.log('    $ fakeApi api.json');
    console.log('    $ fakeApi api.js');
    console.log('    $ fakeApi api.json api2.json api3.js');
    return console.log('    $ fakeApi api.json -p 80');
  });

  commander.parse(process.argv);

  if (_.isEmpty(commander.args)) {
    commander.help();
  }

  logger.isVerbose = commander.verbose !== void 0;

  port = parseInt(commander.port);

  apiFiles = commander.args.map(function(apiFile) {
    return path.resolve(process.cwd(), apiFile);
  });

  apiProvider.init(apiFiles);

  app = express();

  app.all(/^(.+)$/, function(req, res) {
    var dataResult;
    res.header('Access-Control-Allow-Origin', '*');
    path = req.params[0];
    dataResult = apiProvider.getApiData(path, req.method, req.query);
    if (dataResult.found) {
      logger.info("[Hitted] " + dataResult.hitted);
      logger.verbose("[Hitted Result] " + dataResult.result + "\n");
      return res.send(dataResult.result);
    } else {
      logger.info("[Not Found] [" + req.method + "]" + path);
      res.status(404);
      return res.send();
    }
  });

  app.listen(port);

  logger.info("API Faker server listening at port: " + port + "\n");

}).call(this);
